...
#a5: alternatively, with my toothpick placement, 0-pi where 0 points due left, pi/2 is momstly straight, pi is mostly right
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if (state == 'find_arm'):
            ...
            if (blob != None):
                pic = draw_arm(c,d,blob, blob_cent)
                rect = cv2.minAreaRect(blob) #returns (center, size, angle in degrees, 0 - (-90))
                bw,bh = rect[1]
                box = cv2.boxPoints(rect)
                box = np.int0(box)
                cv2.drawContours(pic,[box],0,(0,0,255),1)
                b_area = bw*bh
                area = cv2.contourArea(blob)
                #print b_area, area, (b_area - area) / area
                if (b_area - area) < (0.25 * area):
                    ...
                    cv2.drawContours(pic,[box],0,(0,255,255),2)
                    cv2.putText(pic, 'press <space> to start tracking.', (0,100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0))
                    cv2.putText(pic, 'A:{} (w,h):({},{})'.format(int(rect[2]), int(bw), int(bh)), (0,150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0))
                else:
                    target = None
            else:
                pic = cv2.bitwise_and(c, c)
            ...
            if (blob != None):
                ...
                if (b_area - area) < (0.25 * area):
                    ...
                    p0_2d = rect[0]
                    p0_2d = np.array([p0_2d[0], p0_2d[1]], np.uint)
                    p0 = dev.deproject_pixel_to_point(p0_2d, get_depth(d, p0_2d))
                    p1_2d = p0_2d+np.array([20,0])
                    p1 = dev.deproject_pixel_to_point(p1_2d, get_depth(d, p1_2d))
                    p2_2d = p0_2d+np.array([0,20])
                    p2 = dev.deproject_pixel_to_point(p2_2d, get_depth(d, p2_2d))
                    v1 = p1 - p0
                    ...
                    wrist_angle = asin(v3[1])
                    grip_angle = asin(v3[0])+pi/2
                    p3 = p0 - v3*60
                    p3_2d = dev.project_point_to_pixel(p3)
                    cv2.line(pic, tuple(p0_2d), tuple(p3_2d), (0,255,0))
                    delta = np.array(new_target_T) - np.array(target_T)
                    new_T = np.array(T_init)+np.array(xform_to_arm(delta))
                    b0,b1,b2,b3 = find_angles(new_T,wrist_angle)
                    b4 = rect[2] / 180 * pi
                    if (bh > bw):
                        b4 = -b4
                    move_to_angle(ser, (b0,b1,b2,b3,b4,grip_angle), None, 400)
        else:
            pic = cv2.bitwise_and(c, c)
